...
const AnGrid = forwardRef((props, ref) => {
    ...
    useImperativeHandle(ref, () => ({
        ...
        addRows: (newRows, prepend) => {
            if (prepend) setRows((p) => [...formatData(newRows), ...p])
            else setRows((p) => [...p, ...formatData(newRows)])
        },
        hideRow: (idx) => {
            let tempRows = [...rows]
            let row = {
    ...
    return (
        <div
            className={`angrid ${props.theme || 'dark'} ${
                props.className || null
            } ${rows.length === 0 ? 'is-empty' : null}`}
            style={{
